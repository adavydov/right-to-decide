...
export function deriveTransition(previous, oldEdition, book) {
  ...
  const sourceRevision = ["8.0", "9.0"].includes(book.editionVersion);
  ...
  const oldSelection = oldEdition.chapters.map(c => c.id);
  if (isV9) {
    assert.equal(oldEdition.id, "literary-manuscript-v8.0-f3c9128893a3", "9.0 must preserve the actual published 8.0 baseline");
    assert.ok(!oldSelection.includes("appendix-d"), "Reader appendix ID collides with the published baseline");
    assert.deepEqual(chapters.map(c => c.id), [...oldSelection, "appendix-d"], "9.0 must retain every published section and add only appendix D");
    const appendix = chapters.at(-1);
    assert.equal(appendix.kind, "appendix");
    assert.equal(appendix.version, "1.6");
  } else {
    assert.deepEqual(chapters.map(c => c.id), oldSelection, "The transition must retain the published chapter selection.");
  }
  ...
  for (const chapter of chapters) {
    ...
    if (added) {
      assert.ok(isV9 && chapter.id === "appendix-d" && !oldChapter, "Unexpected new reader section");
    } else {
    ...
    const changed = added || source !== before.source_sha256;
    const ids = {};
    for (const block of sourceBlocks) {
      if (!/^[a-zA-Z0-9][a-zA-Z0-9_-]*$/.test(block.id)) throw new Error("Unsafe reader block identifier.");
      let stable;
      if (changed) {
        if (!block.id.startsWith((isV9 ? "manuscript-v9-" : book.editionVersion === "8.0" ? "manuscript-v8-" : "manuscript-v7-") + chapter.id + "-") || oldIds.has(block.id))
          throw new Error("A rewritten or added chapter must have entirely new edition block identities.");
        stable = block.id;
      } else {
    ...
    transitions.push({ chapter_id: chapter.id, from_source_sha256: added ? null : before.source_sha256, to_source_sha256: source,
      action: added ? "new-chapter-no-annotation-transfer" : changed ? "new-identities-no-annotation-transfer" : "retain-identical-blocks",
      retired_block_ids: changed && !added ? Object.values(before.blocks) : [],
...
export function verifyTransition(root, book, currentMapBytes) {
  ...
  if (!fs.existsSync(local(root, receiptPath))) {
    if (["7.1", "8.0", "9.0"].includes(book.editionVersion)) throw new Error("The literary identity transition requires its explicit receipt.");
  ...
  assert.equal(receipt.strategy, ["8.0", "9.0"].includes(book.editionVersion) ? "new-identities-for-source-revision" : "new-identities-for-literary-rewrite");
  ...
  for (const source of [manifest.prologue, ...manifest.chapters, ...(manifest.supplements || []), manifest.authorContents]) checked(root, source);
  ...
  assert.equal(selection.authorInstruction, book.editionVersion === "9.0" ? "Ну все, все готово, пересобирай книгу и публикуй все" : book.editionVersion === "8.0" ? "пиши новую литературную редакцию и публикуй" : "Обнов сайт");
  ...
  if (["8.0", "9.0"].includes(book.editionVersion)) {
