...
assert.equal(book.editionVersion, "8.0", "Prepare the accepted 8.0 projection first");
...
if (fs.existsSync(path.join(root, receiptPath))) {
  ...
  console.log("V8 IDENTITY RECEIPT CHECK OK: " + editionId);
} else {
  ...
  assert.equal(manifest.editionVersion, "8.0");
  ...
  const receipt = {
    ...
    review_scope: "Technical transition of accepted source bytes. Identical sources retain exact prior blocks; changed sources receive new IDs. No annotation transfer or claim that every chapter was rewritten is made.",
  ...
  verifyTransition(root, book, read(mapPath));
  console.log("V8 IDENTITY RECEIPT PREPARED: " + editionId);
}
