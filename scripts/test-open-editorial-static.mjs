...
test('default static generator is reproducible in a minimal checkout with no service or npm modules',t=>{
 ...
 if(fs.existsSync(path.join(root,receiptPath))){
  ...
  inputs.push(...[release.prologue,...release.chapters,...(release.supplements||[]),release.authorContents].map(item=>item.path));
  if(book.editionVersion==='9.0')assert.ok(selection.inputs.some(item=>item.path.endsWith('/supplements.json')),'The explicit v9 supplement selection is part of the minimal checkout.');
