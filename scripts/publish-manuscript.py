...
def main():
    ...
    parser.add_argument("--edition", choices=["v4", "v6", "v7", "v7.1", "v8", "v9"], help="Literary source directory; check detects the saved release")
    ...
    try:
        ...
        if args.edition == "v9" or (args.edition is None and current.get("editionVersion") == "9.0"):
            import release_v9
            release_v9.main(args, parser, chapter_selection)
            return
        if args.edition is None and current.get("editionVersion") not in {None, "4.0", "5.1", "6.0", "7.0", "7.1", "8.0"}:
            raise ValueError("No verified publisher for saved edition")
        ...
        if use_v7:
            import release_v7
            release_v7.main(args, parser, chapter_selection)
            return
