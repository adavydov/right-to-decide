...
export const metadata = { title: "Исследования и модели", alternates: {canonical: siteConfig.publicUrl + "/research/v10/"} };
...
export default function ResearchV10() {
  if (dossier.status !== "accepted-public-package") notFound();
  const intro = dossier as unknown as Section & {models: Section[]};
  return <main id="main-content" className="subpage wrap"><div style={{maxWidth:800,margin:"0 auto"}}><p className="eyebrow">Редакция {book.editionVersion}</p><h1 className="page-heading">{intro.title}</h1><div className="reading-copy"><Content section={intro} />
