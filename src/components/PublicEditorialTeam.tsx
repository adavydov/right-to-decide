...
export function PublicEditorialTeam({team,compact=false}:{team:Team;compact?:boolean}){
  return <section id="literary-team" aria-labelledby="literary-team-title" className={styles.team}>
    ...
    <div className={styles.links}>{compact&&<Link href="/authors/#literary-team">О редакции ↗</Link>}<Link href="/library/">Библиотека ↗</Link><a href={assetPath('/editorial-team.json')} type="application/json">Описание в JSON ↗</a></div>
  </section>;
}
