...
const groups=[{id:"prologue",title:"Пролог",description:[] as string[],chapters:filtered.filter(c=>!c.part&&c.kind!=="backmatter")},...parts.map(p=>({...p,chapters:filtered.filter(c=>c.part===p.title)})),{id:"epilogue-section",title:"Эпилог",description:[] as string[],chapters:filtered.filter(c=>!c.part&&c.kind==="backmatter")}].filter(g=>g.chapters.length);
