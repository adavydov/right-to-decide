...
import { editorialDate, readBrowserNotes, saveBrowserNotes, loadEditorialCorpus, type EditorialCorpus, type EditorialNote } from "@/lib/open-editorial-local";
...
export function OpenEditorialLocalNotes() {
  ...
  function originalUrl(note: EditorialNote) {
    const edition = corpus?.editions.find(e => e.id === note.target.edition_id);
    const chapter = edition?.chapters.find(c => c.id === note.target.chapter_id);
    if (!edition || (note.target.chapter_id && !chapter)) return undefined;
    const block = chapter?.blocks.find(b => b.id === note.target.block_id);
    if (note.target.block_id && !block) return undefined;
    if (edition.id === "literary-manuscript-v9.0-26472e4aa686") {
      const domId = block && ((block as typeof block & { dom_id?: string }).dom_id || block.id);
      return assetPath("/editions/v9/" + (chapter ? "read/" + chapter.id + "/" : "")) + (domId ? "#" + encodeURIComponent(domId) : "");
    }
    return block?.canonical_url || chapter?.canonical_url || assetPath("/editorial/editions/" + encodeURIComponent(edition.id) + "/index.html");
  }
